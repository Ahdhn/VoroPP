// Build voronoi for a set of vertices.. Inspired by
...
{
   public:
    // Constructor with the container extends from (xmin, ymin, zmin) to (xmax,
    // ymax, zmax)
    Voro(double xmin,
         double xmax,
         double ymin,
        ...
        : m_xmin(xmin), m_xmax(xmax), m_ymin(ymin), m_ymax(ymax), m_zmin(zmin),
          m_zmax(zmax), m_is_computed(false){};

    ~Voro(){};


    // Builds voronoi diagram for the vertices inside the container
    template <typename T>  // examples of T could be double**,
                           // std::vector<std::vector<double>>& or any similar
                           // type that can be indexed twice
...
void Voro::getVoronoi(int num_vertices, T& vertices_lambda)
{
    ...
    // 1) init container
    // use a pre container to get the optimal number of blocks that the
    // container is divided into
    voro::pre_container pcon(m_xmin, m_xmax, m_ymin, m_ymax, m_zmin, m_zmax,
                             false, false, false);

    // 2) add vertices to the pre container (it will be passed then to the main
    // container)
    for (int i = 0; i < num_vertices; i++) {
        double x(vertices_lambda(i, 0)), y(vertices_lambda(i, 1)),
            z(vertices_lambda(i, 2));
        if (x < m_xmin || x > m_xmax || y < m_ymin || y > m_ymax ||
            z < m_zmin || z > m_zmax) {
            std::cout << "ERROR::Voro::getVoronoi():: Vertex not inside the "
                         "bounding box. It will be skipped!!!"
                      << std::endl;
...
void Voro::build(voro::pre_container& pcon)
{
    ...
    voro::container con(m_xmin, m_xmax, m_ymin, m_ymax, m_zmin, m_zmax, nx, ny,
                        nz, false, false, false,
                        8);  // 1st to 6th paramter is the containers
    pcon.setup(con);         // pass the vertices to the (main) container
                      // //7th to 9th parameter is the number of blocks that the
                      // container is divided into
    // 10th to 12 is isPeriodic in x, y or z
    // 13th parameter is allocation space for how many particles within each
    // computational block
    voro::voronoicell_neighbor
        cn;  //(this is just for guidance, the code will dynamically allocate
             //more in case of overflow)
        ...
        do
            if (con.compute_cell(cn, cl)) {
                ...
                std::vector<int> neigh,
                    f_vert;  // my current cell list of neighbours and faces
                std::vector<double> v;  // my current cell's vertices vector

                cn.neighbors(neigh);  // list of neighbours
                cn.face_vertices(
                    f_vert);  // list of the number of vertices per face
                ...
                for (size_t i = 0, j = 0; i < neigh.size();
                     i++, j += f_vert[j] + 1) {
                    if (neigh[i] > 0) {
...
void Voro::drawVoronoi(std::string filename) const
{
    ...
    if (!m_is_computed) {
        std::cout << "ERROR::Voro::drawVoronoi():: There is no faces to draw. "
                     "Call getVoronoi() first!!!"
                  << std::endl;
        return;
    }
    ...
    for (size_t i = 0; i < m_voro_faces.size(); i++) {
        ...
        for (size_t v = 0; v < m_voro_faces[i].size(); v += 3) {
            file << "v	" << m_voro_faces[i][v] << "	"
                 << m_voro_faces[i][v + 1] << "	" << m_voro_faces[i][v + 2]
                 << std::endl;
        }
